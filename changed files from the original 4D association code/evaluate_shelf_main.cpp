...
Eigen::Matrix4Xf MappingToShelf(const Eigen::Matrix4Xf& skel19)
{
	...
	Eigen::Matrix4Xf shelf15(4, 17);

	//const std::vector<int> mapping = { 13, 7, 2, 3, 8, 14, 15, 11, 5, 6, 12, 16, 1, 4, 0 };
	const std::vector<int> mapping = { 1, 4, 0, 6, 12, 16, 3, 8, 14, 5, 11, 15, 2, 7, 13, 10, 9};
...
Eigen::VectorXi Evaluate(const Eigen::Matrix4Xf& shelfSkel, const Eigen::Matrix4Xf& gt)
{
	...
	const SkelDef& def = GetSkelDef(PAN17);
...
void PrintEvaluation(const std::vector<Eigen::VectorXi>& correctJCnt) {
	...
	*/
	...
	std::vector<std::string> pafName = {
		"Neck-Nose", "Neck-Right Shoulder", "Neck-Left Shoulder", "Right Shoulder-Right Hip", "Left Shoulder-Left Hip", 
		"Middle Hip-Right Hip", "Middle Hip-Left Hip", "Right Upper Leg", "Left Upper Leg", "Right Lower Leg", 
		"Left Lower Leg", "Right Upper Arm", "Left Upper Arm", "Right Lower Arm", "Left Lower Arm",
		"Right Eye-Nose", "Right Eye-Nose" };
...
#pragma omp parallel for
	for (int i = 0; i < cams.size(); i++) {
		...
		videos[i] = cv::VideoCapture("../data/shelf/video/" + iter->first + ".avi");
	...
	SkelPainter shelfPainter(PAN17);
